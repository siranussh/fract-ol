...
void	draw_fractal(t_data *fract)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			px_real = fract->min_real + (double)x * (fract->max_real - fract->min_real) / WIDTH;
			px_imaginary = fract->max_imaginary + (double)y * (fract->min_imaginary - fract->max_imaginary) / HEIGHT;
